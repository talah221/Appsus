...
function addGoogleBook(bookFromGoogle) {
    const bookToAdd = {
        id: bookFromGoogle.id,
        title: bookFromGoogle.volumeInfo.title,
        authors: bookFromGoogle.volumeInfo.authors,
        publishedDate: (bookFromGoogle.volumeInfo.publishedDate) ? bookFromGoogle.volumeInfo.publishedDate.substring(0, 4) : 2000,
        categories: bookFromGoogle.volumeInfo.categories,
        description: bookFromGoogle.volumeInfo.description,
        listPrice: {
            isOnSale: true, amount: bookFromGoogle.saleInfo.listPrice ? bookFromGoogle.saleInfo.listPrice.amount : utilService.getRandomNum(),
            currencyCode: bookFromGoogle.saleInfo.listPrice ? bookFromGoogle.saleInfo.listPrice.currencyCode : 'USD'
        },
        pageCount: bookFromGoogle.volumeInfo.pageCount,
        thumbnail: bookFromGoogle.volumeInfo.imageLinks.thumbnail,
        subtitle: bookFromGoogle.volumeInfo.subtitle
    };
...
function query(filterBy) {
    let booksToReturn;
    if (filterBy) {
        booksToReturn = books.filter(book => book.title.toLowerCase().includes(filterBy.name.toLowerCase()))
        booksToReturn = booksToReturn.filter(book => book.listPrice.amount < filterBy.price)

    } else booksToReturn = books
...
function getBookById(bookId) {
    return new Promise((resolve, reject) => {
        ...
        bookToReturn ? resolve(bookToReturn) : reject('Book Doesnt exist')
    })
...
function addReview(id, userReview) {
    const idx = books.findIndex((book) => book.id === id)
    if (!userReview.txt || !userReview.date || !userReview.fullName || !userReview.rate) return;
    userReview.id = gId;
    gId++
    if (!books[idx].reviews) {
        books[idx].reviews = [userReview]

    }
    else {
        books[idx].reviews.push(userReview)
    }
    storageService.saveToStorage('books', books)
...
function deleteReview(reviewId, bookId) {
    ...
    const reviewToDeleteIdx = books[currBookIdx].reviews.findIndex((review) => review.id === reviewId)
    books[currBookIdx].reviews.splice(reviewToDeleteIdx, 1);
    storageService.saveToStorage('books', books)
}


function getResult(userSearch) {
    var prmRes = axios.get(`https://www.googleapis.com/books/v1/volumes?printType=books&q=effective%20${userSearch}`)
